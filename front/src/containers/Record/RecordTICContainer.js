...
import {getTicDetail} from "../../store/modules/video.js"
...
function TICContainer({ match }) {
  const dispatch = useDispatch();
  const key = match.params.key;
  const ticStream = "/record/tic/" + key;
  ...
  if (targetInfo === null && error.type !== "detail"){
    dispatch(getTicDetail(key))
  }
  if (targetInfo && targetInfo.name !== key) {
    dispatch(getTicDetail(key))
  }
      ...
      <div className={classes.mainPanel} ref={mainPanel}>
        ...
        <div className={classes.content}>
          ...
          <CardHeader color="warning">
            <h4 className={cardClasses.cardTitleWhite}>[열화상] 녹화영상</h4>
